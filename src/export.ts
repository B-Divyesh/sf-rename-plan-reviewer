...
export function stageRows(rows: RenameRow[], separator: '/' | '\\' = '/'): StagedRename[] {
  return rows.map((row, index) => {
    const current = canonicalPath(row.current, separator);
    const next = canonicalPath(row.next, separator);
    const slash = current.lastIndexOf(separator);
    const directory = slash >= 0 ? current.slice(0, slash + 1) : '';
    const basename = current.slice(slash + 1);
    return { ...row, current, next, temporary: `${directory}.rpr-${hash(`${current}\u0000${next}\u0000${index}`)}-${basename}` };
  }).filter((row) => row.current && row.next && row.current !== row.next);
}
...
function targetDirectories(staged: StagedRename[], separator: '/' | '\\'): string[] {
  return [...new Set(staged.map((row) => {
    const slash = row.next.lastIndexOf(separator);
    return slash >= 0 ? row.next.slice(0, slash) : '';
  }).filter(Boolean))];
}
